...
public class GiftCardReader extends ConnectSQL{
  ...
  Saver data=new ConnectSQL();
   ...
   public int getLastGiftCard() throws Exception{
    int count=0;
    ArrayList<GiftCard> temp=(data).readGiftCards("Giftcards.txt");
    if(temp.size()!=0){
      GiftCard g=null;
      for(GiftCard t:temp){
        g=t;
      }
      count=Integer.parseInt(g.getGiftCardId());
    }
    return count;
   }
